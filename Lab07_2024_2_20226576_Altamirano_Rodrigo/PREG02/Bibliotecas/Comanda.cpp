...
void Comanda::actualizar() {
    for (int i=0;i<this->cantidad_bebidas;i++) this->total+=this->bebidas[i].get_precio();
    for (int i=0;i<this->cantidad_entradas;i++) this->total+=this->entradas[i].get_precio();
    for (int i=0;i<this->cantidad_platos_fondo;i++) this->total+=this->platos_fondo[i].get_precio();
    if (this->hora_atencion+this->tiempo_preparacion>this->hora_servicio) this->set_estado("ATENDIDA CON RETRASO");
    else this->set_estado("ATENDIDA");
}
