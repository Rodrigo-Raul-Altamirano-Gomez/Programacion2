...
int LibroPrestado::buscarUsuario(const Usuario *arrUsuarios) {
    for (int i=0;arrUsuarios[i].getCarne()>0;i++) {
        if (arrUsuarios[i].getCarne()==this->carne) return i;
    }
    return -1;
}

int LibroPrestado::buscarLibro(const Libro *arrLibros) {
    for (int i=0;arrLibros[i].getPrecio()>0;i++) {
        if (strcmp(arrLibros[i].getCodigo(),this->codigoLib)==0) return i;
    }
    return -1;
}
